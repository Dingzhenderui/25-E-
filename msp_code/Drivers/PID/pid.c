...
{  
    int Bias;  //定义相关变量
		...
		Last_biasA=Bias;	
	    if(ControlVelocityA>100) ControlVelocityA=100;
	    else if(ControlVelocityA<-100) ControlVelocityA=-100;
    ...
    int Bias;  //定义相关变量
		...
		ControlVelocityB+=Velcity_Ki*(Bias-Last_biasB)+Velcity_Kp*Bias;  //增量式PI控制器
                                                                   //Velcity_Kp*(Bias-Last_bias) 作用为限制加速度
	                                                                 //Velcity_Ki*Bias             速度控制值由Bias不断积分得到 偏差越大加速度越大
		Last_biasB=Bias;	
	    if(ControlVelocityB>100) ControlVelocityB=100;
	    else if(ControlVelocityB<-100) ControlVelocityB=-100;
